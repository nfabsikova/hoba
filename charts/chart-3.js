 export function makeChart3(mc, ba, danube, danubeLine) {
  ...
  //Footnote
  let footnote = "Odhad podielu trvalo bývajúceho obyvateľstva nad 65 rokov podľa štvorcov 250x250m, Zdroj: MV SR, Spracoval: Magistrát Hlavného mesta SR Bratislavy";
  ...
  //Create svg
  d3.select("#chart3").append("div").attr("id", "chart3_container");
  let chart = d3.select("#chart3_container")
    .append("svg")
    .attr("width", w)
    .attr("height", h);

  //Create tooltip
  let tooltip = d3.select("#chart3_container").append("div").attr("id", "tooltip3").attr("class", "hidden");
  tooltip.append("p").append("span").attr("id", "value3").attr("class", "numbers").text("100");

  //Create footnote
  d3.select("#chart3").append("hr")
  d3.select("#chart3").append("p").attr("class", "footnote").text(footnote)
